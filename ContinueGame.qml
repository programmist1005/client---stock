            ...
            model: DelegateModel {
                ...
                model: ListModel {
                    ...
                    Component.onCompleted: {
                        ...
                        for (i = 0; i<8; ++i){
                            for (j = 0; j<8; ++j){
                                append({ sourse: moveFigure.getFigureHistory(i,j), i:i, j:j });
                            }
                        }
                    }
                }
                ...
                delegate: MouseArea {
                    ...
                    DropArea {
                        ...
                        WhiteFigure {
                            ...
                            onSignalExit: {
                                sourseListModel.setProperty(mouseArea.visualIndex, "sourse", moveFigure.replacePawn());
                                whiteFigure.close()
                                continueGame.show()
                            }
                        }
                        ...
                        onDropped: {
                            ...
                            sourseListModel.setProperty(drag.source.visualIndex, "sourse", "");
                            sourseListModel.setProperty(moveFigure.move(dragSourse, dragNumber, dragI, dragJ, targetSourse, targetNumber, targetI, targetJ), "sourse", dragSourse);

                            if( moveFigure.take() > 0 ){
                                sourseListModel.setProperty(moveFigure.take(), "sourse", "");
                            }

                            if( moveFigure.replace() === 1 ){
                                whiteFigure.show()  // Открываем окно с белыми фигурами
                                continueGame.hide()   // Скрываем основное окно
                            }

                            if( moveFigure.replace() === 2 ){
            ...
            model: DelegateModel {
                ...
                delegate: MouseArea {
                    ...
                    Text {
                        ...
                        color: "red"
                        text: model.type //"["+model.i+"]" + "["+model.j+"]"//visualIndex
                    }
